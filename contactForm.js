...
function dm(n) {
    ...
    case n.classList.contains('hi') || n.classList.contains('hil') || n.classList.contains('vi') || n.classList.contains('vil') || n.classList.contains('si'):
        if(n.classList.contains('animatedImg')){
            ...
            // if(mdWidth<width || mdHeight<height){}
...
function updateAnimatedImgsModal(n,animatedImg){
    ...
    var mdWidth=window.getComputedStyle(mg).width.slice(0,-2);
    var mdHeight=window.getComputedStyle(mg).height.slice(0,-2);
    var mgWidth=window.getComputedStyle(mg).width.slice(0,-2);
    var mgHeight=window.getComputedStyle(mg).height.slice(0,-2);
    if(animatedImg.width>=animatedImg.height){
        mg.style.height = parseFloat(window.getComputedStyle(mg).width.slice(0,-2)) * inverseAspectRatio + 'px';
        mg.style.width = parseFloat(window.getComputedStyle(md).height.slice(0,-2)) * animatedImg.width/animatedImg.height + 'px';
    }
    else if(animatedImg.width<animatedImg.height){
        mg.style.width = parseFloat(window.getComputedStyle(mg).height.slice(0,-2)) * animatedImg.width/animatedImg.height + 'px';
        mg.style.height = parseFloat(window.getComputedStyle(md).width.slice(0,-2)) * inverseAspectRatio + 'px';
    }
    else{console.log('animatedImg modal error');}
    var curTime = performance.now();
