...
function fiChange(){
    for(i=0;i<fi.length;i++){
        if(fi[i].value===""){
            ...
            if(captchaRendered===false){grecaptcha.enterprise.render('fc',{'sitekey':'',});fcHeight=fc.getBoundingClientRect().height;captchaRendered=true;}
...
function formSent(){
    ...
    let css = '#fs:hover{transition:background-color .25s;background-color:rgba(0,51,0,1);}#fs:active{transition:background-color .25s;background-color:rgba(0,51,0,1);}';let styleTag = document.createElement('style');styleTag.appendChild(document.createTextNode(css));document.getElementsByTagName('head')[0].appendChild(styleTag);
